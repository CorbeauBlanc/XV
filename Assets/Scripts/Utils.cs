...
public abstract class GoodBehaviour : MonoBehaviour
{
	...
	public Dictionary<string, Action<GameObject>> afterInstantiationCallbacks = new Dictionary<string, Action<GameObject>>();
	...
	private static void InitializeClone(GameObject original, GameObject clone)
	{
		...
		{
			behaviour.afterInstantiationCallbacks.Values.ForEach((Action<GameObject> callback) =>
			{
				callback(clone);
			});
		});
